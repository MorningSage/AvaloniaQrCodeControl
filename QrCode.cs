...
public class QrCode : Control
{
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
    {
        ...
        switch (change.Property.Name)
        {
            ...
            case nameof(Data):
                lock(_setBitsTable) _setBitsTable.Clear();
                OnDataChanged(QrCodeGenerator.CreateQrCode(Data, ToQrCoderEccLevel(ErrorCorrection)));
                ...
                if (_transitionTask == null || _transitionTask.IsCompleted)
                {
                    _transitionTask = Dispatcher.UIThread.Invoke(async () =>
                    {
                        ...
                        {
                            if (_qrCodeGeometry is var (newGeometry, newOpacity)) _qrCodeGeometry = (newGeometry, Math.Min(1, newOpacity + 0.1));
                            InvalidateVisual();
                            await Task.Delay(30);
                        }
    ...
    private void OnDataChanged(QRCodeData qrCodeData)
    {
        /*
        ...
        var columnCount = matrix[0].Count - QuietMargin;
        var rowCount    = matrix.Count    - QuietMargin;
        ...
        for (var row = 0; row < matrix.Count; row++)
        {
            ...
            if (row < QuietZoneCount || row > matrix.Count - QuietZoneCount - 1) continue;
    ...
    private void ProcessRow(PathGeometry geometry, IReadOnlyList<BitArray> bitMatrix, int row, Size symbolSize)
    {
        // Loop through each item within the row
        for (var column = 0; column < bitMatrix[row].Count; column++)
        {
            // Quiet Space - helper not used to speed up processing
            if (column < QuietZoneCount || column > bitMatrix[row].Count - QuietZoneCount - 1) continue;
    ...
    /// <param name="symbolSize">The calculated size of each symbol</param>
    private void ProcessSymbol(PathGeometry geometry, IReadOnlyList<BitArray> bitMatrix, int row, int column, Size symbolSize)
    {
        ...
        if (ProcessSymbolIfSet(geometry, bitMatrix, row, column, symbolBounds)) return;
    ...
    private bool ProcessSymbolIfSet(PathGeometry geometry, IReadOnlyList<BitArray> bitMatrix, int row, int column, Rect symbolBounds)
    {
        // If not filled, no action required
        if (!IsValid(bitMatrix, column, row)) return false;
    ...
    private CornerFlags GetSetSymbolCornerFlags(IReadOnlyList<BitArray> bitMatrix, int row, int column)
    {
        var flags = CornerFlags.None;

        if (!IsValid(bitMatrix, column, row)) return flags;
        
        if (IsValid(bitMatrix, column, row - 1) || IsValid(bitMatrix, column - 1, row)) flags |= CornerFlags.TopLeft;
        if (IsValid(bitMatrix, column, row - 1) || IsValid(bitMatrix, column + 1, row)) flags |= CornerFlags.TopRight;
        if (IsValid(bitMatrix, column, row + 1) || IsValid(bitMatrix, column + 1, row)) flags |= CornerFlags.BottomRight;
        if (IsValid(bitMatrix, column, row + 1) || IsValid(bitMatrix, column - 1, row)) flags |= CornerFlags.BottomLeft;
    ...
    /// <param name="column">The column of the symbol being processed</param>
    /// <param name="symbolBounds">The bounds of the symbol being processed</param>
    private void ProcessSymbolIfUnset(PathGeometry geometry, IReadOnlyList<BitArray> bitMatrix, int row, int column, Rect symbolBounds)
    {
        // If filled, no action required
        if (IsValid(bitMatrix, column, row)) return;
        ...
        if (cornerFlags == CornerFlags.None) return;
    ...
    private CornerFlags GetUnsetSymbolCornerFlags(IReadOnlyList<BitArray> bitMatrix, int row, int column)
    {
        var flags = CornerFlags.None;

        if (IsValid(bitMatrix, column, row)) return flags;
        
        if (IsValid(bitMatrix, column, row - 1) && IsValid(bitMatrix, column - 1, row - 1) && IsValid(bitMatrix, column - 1, row)) flags |= CornerFlags.TopLeft;
        if (IsValid(bitMatrix, column, row - 1) && IsValid(bitMatrix, column + 1, row - 1) && IsValid(bitMatrix, column + 1, row)) flags |= CornerFlags.TopRight;
        if (IsValid(bitMatrix, column, row + 1) && IsValid(bitMatrix, column + 1, row + 1) && IsValid(bitMatrix, column + 1, row)) flags |= CornerFlags.BottomRight;
        if (IsValid(bitMatrix, column, row + 1) && IsValid(bitMatrix, column - 1, row + 1) && IsValid(bitMatrix, column - 1, row)) flags |= CornerFlags.BottomLeft;
    ...
    /// <param name="y"></param>
    /// <returns></returns>
    private bool IsValid(IReadOnlyList<BitArray> bitMatrix, int x, int y)
    {
        var key = (x, y).GetHashCode();

        lock (_setBitsTable)
        {
            if (_setBitsTable.ContainsKey(key)) return (bool)_setBitsTable[key]!; 
            
            // Top Left Marker
            if (x < QuietZoneCount + 8 && y < QuietZoneCount + 8) return (bool)(_setBitsTable[key] = false);
            // Top Right Marker
            if (x > bitMatrix[y].Count - QuietZoneCount - 9 && y < QuietZoneCount + 8) return (bool)(_setBitsTable[key] = false);
            // Bottom Left Marker
            if (x < QuietZoneCount + 8 && y > bitMatrix.Count - QuietZoneCount - 9) return (bool)(_setBitsTable[key] = false);
    ...
    public override void Render(DrawingContext context)
    {
        ...
        if (_qrCodeGeometry == null) return;
    ...
    protected static QRCodeGenerator.ECCLevel ToQrCoderEccLevel(EccLevel eccLevel)
    {
        return eccLevel switch
        {
            EccLevel.Lowest  => QRCodeGenerator.ECCLevel.L,
            EccLevel.Medium  => QRCodeGenerator.ECCLevel.M,
            EccLevel.Quality => QRCodeGenerator.ECCLevel.Q,
            EccLevel.Highest => QRCodeGenerator.ECCLevel.H,
